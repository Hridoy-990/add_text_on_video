package com.example.add_text_on_video.processing
...
import com.example.add_text_on_video.MainActivity
import com.example.add_text_on_video.getSupportedVideoSize
import com.example.add_text_on_video.render.TextAnimator
import com.example.add_text_on_video.render.TextureRenderer
 ...
 */
class AddTextToVideoProcessing {
     ...
     *             Feel free to adjust to your preferences.
       MediaExtractor -  to extract metadata and encoded frames from video file
       MediaCodec - Encode and decode
       MediaMuxer - Save the encoded video in proper container format into a file using MediaMuxer
     */
    ...
    dequeueOutputBuffer - Dequeue an output buffer, block at most "timeoutUs" microseconds.
                          Returns the index of an output buffer that has been successfully decoded or one of the INFO_* constants.
    writeSampleData  - Writes an encoded sample into the muxer
    releaseOutputBuffer - If you are done with a buffer, use this call to update its surface timestamp and return it to the codec to render it on the output surface.
